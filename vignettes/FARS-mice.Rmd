...
outside any vehicle. The variable `PER_TYP` is the role the person 
...
This generally holds, although some imputed values for unknown `ALC_RES` do not vary.
...
Roughly speaking,
`ST_CASE` is 6 digits (the first two are the U.S. state), `VEH_NO` is two digits,
and `PER_NO` is two digits. We can thus produce an identifier stringing the three
together:
...
of `mice::complete(..., action = 'long', include = TRUE)`, so it would need to have
all of the variables in the data set. Thus we replicate
each line in the data set $M$ times (which is derived from the `miper_long` dataset.)
...
```{r person_as_mids}
as.mids(
  long  = person_long %>% arrange(imp_index, person_id),
